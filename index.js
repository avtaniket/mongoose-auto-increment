...
exports.plugin = function (schema, options) {
  ...
  .then(function(counter) {
    if (!counter) {
      // If no counter exists, create one and save it
      ...
      return counter.save();  // Save the counter and return the promise
    } else {
      ready = true;  // Set ready to true if the counter exists
      return Promise.resolve(); // Resolve a promise to continue chain
    }
  })
  .then(function() {
    ready = true;  // Set ready to true once the counter is saved or exists
  })
  .catch(function(err) {
    console.log('Error while processing identity counter', err);
  ...
  var nextCount = function (callback) {
    IdentityCounter.findOne({
      ...
      .then(function(counter) {
        // Check if counter doesn't exist, return the startAt value, otherwise increment the count
      ...
      .catch(function(err) {
        callback(err);  // Handle any errors
      });
  ...
  var resetCount = function (callback) {
    IdentityCounter.findOneAndUpdate(
      ...
      .then(function(counter) {
        // If no error occurs, return the startAt value to the callback
        ...
        if (ready) {
          ...
          if (typeof doc[settings.field] === 'number') {
            IdentityCounter.findOneAndUpdate(
              ...
              .then(() => {
                // Continue with the default document save functionality after the update is successful.
                next();
              })
              .catch((err) => {
                // Handle any errors.
            ...
            IdentityCounter.findOneAndUpdate(
              ...
              .then((updatedIdentityCounter) => {
                // If the update is successful, set the document's field to the current count.
